...
const NewMovieCard = ({ movies, genres, Index }) => {
  ...
  const handleCardClick = () => {
    const type = movies.media_type || "movie";
    navigate(`/pageDetails/${movies.id}?type=${type}`);
  };

  return (
    <div onClick={handleCardClick} className="movie-card w-80 h-auto flex flex-col group mx-12 shrink-0 group relative">
      <div className="relative w-full overflow-hidden h-44 flex shrink-0">
        ...
        {showTrailer && (
          <CardTrailerModal trailerKey={trailerKey} onClose={() => setShowTrailer(false)} />
        )}
      ...
      <div className="movie-details hidden w-full h-auto bg-c-black text-c-l-grey group-hover:flex flex-col transition-all delay-150 duration-300 ease-in-out space-y-1.5 pl-0.5 overflow-visible">
        ...
        <div className="meta-info flex flex-row flex-wrap space-x-3.5">
          <p className="movie-rating bg-c-d-grey rounded-lg px-1.5 text-c-l-grey">
            {movies.certification || rating || "Not Rated"}
          </p>
          <p className="movie-duration text-c-l-grey bg-c-d-grey rounded-lg px-1.5">
            {formatDateWithDot(movies.first_air_date || movies.release_date)}
          </p>
